...
class Tools
{
	...
	{
		$output = preg_replace_callback('#(?:href|action|data|src)=(["\'])(.+?)\\1#is', 'Tools::parseURLCallback', $output);
		$output = preg_replace_callback('#(?:url)\s*:\s*(["\'])(.+?)\\1#is', 'Tools::parseURLCallback', $output);
		
		$output = preg_replace_callback('#\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}\b#is', 'Tools::parseEmailCallback', $output);
